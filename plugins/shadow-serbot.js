...
shadowJBOptions.mcode = true 
shadowJBOptions.fromCommand = true
shadowJadiBot(shadowJBOptions)
global.db.data.users[m.sender].Subs = new Date * 1
}
handler.help = ['codebot', 'code']
handler.tags = ['serbot']
handler.command = ['codebot', 'code']
export default handler 

export async function shadowJadiBot(options) {
let { pathShadowJadiBot, m, conn, args, usedPrefix, command } = options

const mcode = options.mcode || args[0] && /(--code|code)/.test(args[0].trim()) ? true : args[1] && /(--code|code)/.test(args[1].trim()) ? true : false
